...
with st.expander("Calendar Heatmap", expanded=False):
    active_habits_for_calendar = get_active_habits()
    if not active_habits_for_calendar:
        ...
        grouped_calendar_habits = {key: [] for key, _ in REMINDER_BUCKET_OPTIONS}
        for habit in active_habits_for_calendar:
            grouped_calendar_habits[normalize_reminder_bucket(habit["reminder_bucket"] if "reminder_bucket" in habit.keys() else "anytime")].append(habit)

        for bucket_key, bucket_label in REMINDER_BUCKET_OPTIONS:
            habits_in_bucket = grouped_calendar_habits.get(bucket_key, [])
            if not habits_in_bucket:
                continue

            st.markdown(f"### {bucket_label}")
            for habit in habits_in_bucket:
                rule_text = format_rule_text(
                    habit["habit_type"] or "count",
                    habit["frequency_type"] or "daily",
                    int(habit["frequency_value"] or 1),
                    int(habit["target_count"] or habit["daily_target"] or 1),
                )
                st.markdown('<div class="compact-card">', unsafe_allow_html=True)
                st.markdown(f'<div class="compact-title">{habit["name"]}</div>', unsafe_allow_html=True)
                schedule_text = format_schedule_text(
                    (habit["schedule_mode"] if "schedule_mode" in habit.keys() else "none") or "none",
                    (habit["scheduled_days"] if "scheduled_days" in habit.keys() else "") or "",
                )
                reminder_label = format_reminder_bucket_label(habit["reminder_bucket"] if "reminder_bucket" in habit.keys() else "anytime")
                st.markdown(
                    f"""
                    <div class="chip-row">
                ...
                st.markdown(
                    build_calendar_heatmap_html(habit, selected_month["start"], selected_month["end"]),
                    unsafe_allow_html=True,
                )
                st.markdown(f'<div class="calendar-note">{get_calendar_note_text(habit)}</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
