...
def add_habit(
    ...
    cur.execute("""
        INSERT INTO habits (
            ...
            daily_target, habit_type, frequency_type, frequency_value, target_count, schedule_mode, scheduled_days, reminder_bucket, habit_group
        )
        VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        clean_name,
...
.compact-card {
    ...
    border-radius: 18px;
    padding: 0.9rem 0.95rem 0.76rem 0.95rem;
    margin-bottom: 0.9rem;
    background: rgba(255,255,255,0.98);
    box-shadow: 0 10px 24px rgba(15, 23, 42, 0.035), 0 2px 6px rgba(15, 23, 42, 0.025);
}
...
.edit-box {
    margin-top: 0.14rem;
    padding: 0.48rem 0.5rem 0.14rem 0.5rem;
    border-radius: 14px;
...
@media (max-width: 640px) {
    ...
    .compact-card {
        padding: 0.62rem 0.66rem 0.5rem 0.66rem;
        border-radius: 15px;
    }
    .stButton > button,
    [data-testid="stFormSubmitButton"] > button {
...
with st.expander("Current Progress", expanded=True):
    ...
    if not progress_rows:
        ...
        for habit_group_label, group_rows in grouped_by_habit_group.items():
            with st.expander(f"{habit_group_label}  {len(group_rows)} habit{('s' if len(group_rows) != 1 else '')}", expanded=True):
                ...
                for bucket_key, bucket_label in REMINDER_BUCKET_OPTIONS:
                    ...
                    for row in bucket_rows:
                        ...
                        card_class = "card-even" if idx % 2 == 0 else "card-odd"
                        with st.container(border=True):
                            ...
                            st.markdown(
                                f'<div class="habit-card-inner {card_class}">',
                                unsafe_allow_html=True,
                            )
                            st.markdown(
                            ...
                            st.markdown(
                                ...
                                <div class="chip-row">
                                    ...
                                    <span class="chip">{row["habit_group"]}</span>
                                    <span class="chip">{row["reminder_label"]}</span>
                                    <span class="chip">{period_short}</span>
                                    <span class="chip">{current_count} / {target}</span>
                                    {f'<span class="chip">{row["schedule_text"]}</span>' if row["schedule_text"] else ''}
                                </div>
                                """,
                                unsafe_allow_html=True
                            )
        
                            if row["schedule_status"]:
                                st.markdown(f'<div class="action-note">Schedule: {row["schedule_status"]}</div>', unsafe_allow_html=True)
                            ...
                            if is_count_habit:
                                ...
                                with action_col1:
                                    if st.button("Check in +1", key=f"log_{row['habit_id']}", width="stretch", type="primary"):
                                        log_habit(row["habit_id"], count=1)
                                        st.rerun()
                                with action_col2:
                                    if st.button("Yesterday +1", key=f"log_yesterday_{row['habit_id']}", width="stretch"):
                                        log_habit(row["habit_id"], count=1, log_date=date.today() - timedelta(days=1))
                                        st.rerun()
                                ...
                                with st.expander("Pick a date", expanded=False):
                                    custom_date = st.date_input(
                                        ...
                                        min_value=get_created_date_from_habit(get_habit_by_id(row["habit_id"])),
                                        max_value=date.today(),
                                        key=f"custom_log_date_{row['habit_id']}",
                                    )
                                    if st.button("Log +1 for selected date", key=f"log_custom_{row['habit_id']}", width="stretch"):
                                        log_habit(row["habit_id"], count=1, log_date=custom_date)
                                        st.rerun()
                                st.markdown('<div class="action-note">Quick backfill for yesterday, or choose any past date.</div>', unsafe_allow_html=True)
                            elif is_single_completion:
                                action_col1, action_col2, action_col3 = st.columns([2.2, 1.8, 1.2])
                                with action_col1:
                                    if done_this_period:
                                        st.button("Done", key=f"done_{row['habit_id']}", width="stretch", disabled=True)
                                    else:
                                        if st.button("Mark done", key=f"log_{row['habit_id']}", width="stretch", type="primary"):
                                            error = log_completion_once_for_current_period(row["habit_id"])
                                            if error:
                                                st.info(error)
                                            st.rerun()
                                with action_col2:
                                    if st.button("Done yesterday", key=f"log_yesterday_{row['habit_id']}", width="stretch"):
                                        error = log_completion_once_for_date(row["habit_id"], date.today() - timedelta(days=1))
                                        if error:
                                            st.info(error)
                                        st.rerun()
                                with action_col3:
                                    if st.button("Undo", key=f"undo_{row['habit_id']}", width="stretch", disabled=not done_this_period, type="secondary"):
                                ...
                                with st.expander("Pick a date", expanded=False):
                                    custom_done_date = st.date_input(
                                        "Mark done for",
                                        value=(date.today() - timedelta(days=1)),
                                        min_value=get_created_date_from_habit(get_habit_by_id(row["habit_id"])),
                                        max_value=date.today(),
                                        key=f"custom_done_date_{row['habit_id']}",
                                    )
                                    if st.button("Mark done for selected date", key=f"log_custom_{row['habit_id']}", width="stretch"):
                                        error = log_completion_once_for_date(row["habit_id"], custom_done_date)
                                        if error:
                                            st.info(error)
                                        st.rerun()
                                st.markdown(
                                    f'<div class="action-note">{("Completed for this period" if done_this_period else "Not done yet for this period")} You can backfill yesterday or choose any past date.</div>',
                                    unsafe_allow_html=True,
                                )
                            elif is_multi_completion:
                                action_col1, action_col2 = st.columns(2)
                                with action_col1:
                                    if st.button("Log session", key=f"log_{row['habit_id']}", width="stretch", type="primary"):
                                        log_habit(row["habit_id"], count=1)
                                        st.rerun()
                                with action_col2:
                                    if st.button("Yesterday session", key=f"log_yesterday_{row['habit_id']}", width="stretch"):
                                        log_habit(row["habit_id"], count=1, log_date=date.today() - timedelta(days=1))
                                        st.rerun()
    
                                with st.expander("Pick a date", expanded=False):
                                    custom_session_date = st.date_input(
                                        "Log session for",
                                        value=(date.today() - timedelta(days=1)),
                                        min_value=get_created_date_from_habit(get_habit_by_id(row["habit_id"])),
                                        max_value=date.today(),
                                        key=f"custom_session_date_{row['habit_id']}",
                                    )
                                    if st.button("Log session for selected date", key=f"log_custom_{row['habit_id']}", width="stretch"):
                                        log_habit(row["habit_id"], count=1, log_date=custom_session_date)
                                        st.rerun()
                                st.markdown('<div class="action-note">Each tap records one completion event. Yesterday and custom-date backfill are supported.</div>', unsafe_allow_html=True)
                            ...
                            with st.expander("Manage", expanded=False):
                                ...
                                if edit_frequency_type in {"x_per_week", "weekly"}:
                                    ...
                                    if st.session_state[schedule_enabled_key]:
                                        ...
                                        if edit_frequency_type == "weekly" and len(st.session_state[schedule_days_key]) > 1:
                                            st.caption("Weekly habits use the first selected weekday as the due day.")
                                else:
...
st.markdown('<div class="analytics-title">Analytics</div>', unsafe_allow_html=True)
if not analytics_habits:
    st.markdown('<div class="analytics-note">No habits available for analytics yet.</div>', unsafe_allow_html=True)
else:
    st.markdown('<div class="analytics-note">Pick one habit and both sections below will update together.</div>', unsafe_allow_html=True)
    ...
    st.markdown(
        f"""
        <div class="chip-row" style="margin-top: 0.05rem; margin-bottom: 0.3rem;">
...
with st.expander("Calendar Heatmap", expanded=False):
    if not analytics_habits:
        st.write("No habits available for calendar view yet.")
    else:
        if selected_analytics_habit is None:
            selected_analytics_habit = analytics_habit_map[st.session_state.get("analytics_selected_habit_id", analytics_options[0])]
        ...
        st.markdown(
            ...
            "<span class='calendar-legend-item'><span class='calendar-swatch' style='background: rgba(249,250,251,0.92);'></span>No activity</span>"
            "<span class='calendar-legend-item'><span class='calendar-swatch' style='background: rgba(96, 165, 250, 0.12);'></span>Some progress</span>"
            "<span class='calendar-legend-item'><span class='calendar-swatch' style='background: rgba(34, 197, 94, 0.14);'></span>Strong day / target met</span>"
            "</div>",
            unsafe_allow_html=True,
        )
