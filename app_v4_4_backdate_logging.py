                    ...
                    if is_count_habit:
                        ...
                        with action_col2:
                            if st.button("Yesterday +1", key=f"log_yesterday_{row['habit_id']}", width="stretch"):
                                ...
                                st.rerun()
                        st.markdown('<div class="action-note">Quick backfill for missed check-ins.</div>', unsafe_allow_html=True)
                        ...
                        with action_col3:
                            if st.button("Undo", key=f"undo_{row['habit_id']}", width="stretch", disabled=not done_this_period):
                                ...
                                st.rerun()
                        st.markdown(
                            f'<div class="action-note">{("Completed for this period" if done_this_period else "Not done yet for this period")} You can also backfill yesterday.</div>',
                            unsafe_allow_html=True,
                        )
                        ...
                        with action_col2:
                            if st.button("Yesterday session", key=f"log_yesterday_{row['habit_id']}", width="stretch"):
                                log_habit(row["habit_id"], count=1, log_date=date.today() - timedelta(days=1))
                                st.rerun()
                        st.markdown('<div class="action-note">Each tap records one completion event. Yesterday backfill is supported.</div>', unsafe_allow_html=True)
