...
def get_monthly_stats():
    ...
    cur.execute("""
        SELECT
            ...
            h.frequency_type AS frequency_type,
            h.frequency_value AS frequency_value,
            h.target_count AS target_count,
        ...
        WHERE h.is_active = 1
        GROUP BY
            h.id, h.name, h.habit_type, h.frequency_type, h.frequency_value, h.target_count
    ...
    for row in rows:
        ...
        stats.append({
            ...
            "habit_type": row["habit_type"] or "count",
            "frequency_type": row["frequency_type"] or "daily",
            "frequency_value": int(row["frequency_value"] or 1),
...
with st.expander("Monthly Statistics", expanded=False):
    ...
    if not monthly_stats:
        ...
        st.caption("Note: monthly stats are still mostly v1-style. Weekly streak/stat logic can be upgraded in the next step.")
