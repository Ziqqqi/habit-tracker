...
with st.expander("Add a New Habit", expanded=False):
    ...
    with st.form("add_habit_form", clear_on_submit=True):
        ...
        submitted = st.form_submit_button("Add habit", width="stretch")
            ...
            for row in bucket_rows:
                ...
                with st.container():
                    ...
                    if is_count_habit:
                        ...
                        with action_col1:
                            if st.button("Check in +1", key=f"log_{row['habit_id']}", width="stretch"):
                        ...
                        with action_col1:
                            if done_this_period:
                                ...
                                if st.button("Mark done", key=f"log_{row['habit_id']}", width="stretch"):
                        ...
                        with action_col2:
                            if st.button("Done yesterday", key=f"log_yesterday_{row['habit_id']}", width="stretch"):
                                ...
                                st.rerun()
                        with action_col3:
                            if st.button("Undo", key=f"undo_{row['habit_id']}", width="stretch", disabled=not done_this_period):
                        ...
                        action_col1, action_col2 = st.columns(2)
                        with action_col1:
                            if st.button("Log session", key=f"log_{row['habit_id']}", width="stretch"):
                                log_habit(row["habit_id"], count=1)
                                st.rerun()
                        with action_col2:
                    ...
                    with st.expander("Manage", expanded=False):
                        ...
                        with save_col1:
                            save_clicked = st.button("Save", key=f"save_manage_{habit_id}", width="stretch")
                        with save_col2:
                            close_clicked = st.button("Close", key=f"close_manage_{habit_id}", width="stretch")
                        with save_col3:
                            hide_clicked = st.button("Hide habit", key=f"hide_manage_{habit_id}", width="stretch")
...
with st.expander("Recent log details", expanded=False):
    ...
    if not today_logs:
        ...
        for log in today_logs:
            ...
            with right:
                if st.button("✕", key=f"delete_{log['id']}", width="stretch"):
                    delete_log(log["id"])
                    st.rerun()
...
if not analytics_habits:
    st.caption("No habits available for analytics yet.")
else:
    st.caption("Pick one habit and both sections below will update together.")
    ...
    st.markdown(
        f"""
        <div class="chip-row" style="margin-top: -0.1rem; margin-bottom: 0.4rem;">
