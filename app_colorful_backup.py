...
def get_theme_class(frequency_type: str, habit_type: str) -> str:
    ...
    if frequency_type == "every_n_days":
        return "theme-cycle"
    if habit_type == "completion":
        return "theme-completion"
    return "theme-default"


def get_status_badge_class(progress: float) -> str:
    if progress >= 1:
        return "badge-done"
    if progress >= 0.5:
        return "badge-mid"
...
def format_last_checkin_label(iso_ts: str | None) -> str:
    if not iso_ts:
        return "—"
...
.stApp {
    background:
        radial-gradient(circle at top left, rgba(168, 85, 247, 0.10), transparent 28%),
        radial-gradient(circle at top right, rgba(59, 130, 246, 0.10), transparent 30%),
        linear-gradient(180deg, rgba(248, 250, 252, 0.98) 0%, rgba(244, 247, 251, 0.98) 100%);
}
...
.compact-card {
    position: relative;
    border-radius: 20px;
    padding: 0.85rem 0.9rem 0.72rem 0.9rem;
...
.chip-accent {
    background: linear-gradient(90deg, rgba(224, 231, 255, 0.95), rgba(243, 232, 255, 0.95));
    color: #4338ca;
...
.badge-done {
    background: linear-gradient(90deg, rgba(220, 252, 231, 0.98), rgba(209, 250, 229, 0.98));
    color: #047857;
}

.badge-mid {
    background: linear-gradient(90deg, rgba(254, 249, 195, 0.98), rgba(254, 240, 138, 0.98));
    color: #a16207;
}

.badge-start {
    background: linear-gradient(90deg, rgba(224, 231, 255, 0.98), rgba(224, 242, 254, 0.98));
    color: #1d4ed8;
}
...
.edit-box {
    margin-top: 0.35rem;
    padding: 0.55rem 0.58rem 0.22rem 0.58rem;
    border-radius: 16px;
...
.hero-banner {
    ...
    border-radius: 22px;
    background: linear-gradient(135deg, rgba(99, 102, 241, 0.12), rgba(168, 85, 247, 0.12), rgba(34, 197, 94, 0.10));
...
@media (max-width: 640px) {
    ...
    .compact-card {
        padding: 0.74rem 0.72rem 0.62rem 0.72rem;
        border-radius: 18px;
    }
    .stButton > button {
        font-size: 0.89rem;
...
st.caption("Flexible habit tracking with period-aware progress, cleaner mobile actions, and compact analytics.")
st.markdown('''
<div class="hero-banner">
    <div class="hero-title">A brighter v3.4 look</div>
    <div class="hero-text">More color, softer cards, and clearer visual cues — while keeping the app compact and phone-friendly.</div>
</div>
''', unsafe_allow_html=True)
...
if not progress_rows:
    ...
    for row in progress_rows:
        ...
        with st.container():
            theme_class = get_theme_class(row["frequency_type"], row["habit_type"])
            progress_badge_class = get_status_badge_class(progress)
            st.markdown(f'<div class="compact-card {theme_class}">', unsafe_allow_html=True)
            st.markdown(f'<div class="compact-title">{row["habit_name"]}</div>', unsafe_allow_html=True)
            st.markdown(
                f"""
                <div class="chip-row">
                    <span class="chip chip-accent">{row["rule_text"]}</span>
                    <span class="chip chip-soft">{period_short}</span>
                    <span class="chip chip-progress {progress_badge_class}">{current_count} / {target}</span>
                </div>
                """,
                unsafe_allow_html=True
            )
            ...
            with meta_col2:
                if row["last_logged_at"]:
                    last_time = format_last_checkin_label(row["last_logged_at"])
            ...
            action_col, manage_col = st.columns([1.8, 1])
            with action_col:
                action_label = "Check in +1" if row["habit_type"] == "count" else "Mark done"
                if st.button(action_label, key=f"log_{row['habit_id']}", use_container_width=True):
                    log_habit(row["habit_id"], count=1)
                    st.rerun()
            with manage_col:
                if st.button("Manage", key=f"edit_toggle_{row['habit_id']}", use_container_width=True):
                    st.session_state[f"editing_{row['habit_id']}"] = not st.session_state.get(f"editing_{row['habit_id']}", False)
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('<div class="action-note">Quick action on the left. Manage opens edit and hide options.</div>', unsafe_allow_html=True)

            if st.session_state.get(f"editing_{row['habit_id']}", False):
                st.markdown('<div class="manage-box">', unsafe_allow_html=True)
                st.markdown('<div class="compact-subtle">Manage habit</div>', unsafe_allow_html=True)
                st.markdown('<div class="edit-box">', unsafe_allow_html=True)

                with st.form(f"edit_form_{row['habit_id']}"):
                    ...
                    with save_col2:
                        cancel_clicked = st.form_submit_button("Close", use_container_width=True)
                    with save_col3:
                        hide_clicked = st.form_submit_button("Hide habit", use_container_width=True)
...
with st.expander("Recent log details", expanded=False):
    ...
    if not today_logs:
        ...
        for log in today_logs:
            log_time = datetime.fromisoformat(log["logged_at"]).strftime("%m-%d %H:%M")
